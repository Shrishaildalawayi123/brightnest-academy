...
public class ChatbotService {
        ...
        private static final long AWAITING_GRADE_TTL_MINUTES = 30;
        private static final int MAX_AWAITING_GRADE_SESSIONS = 10_000;

        private static final String WHATSAPP_URL = "https://wa.me/917204193980?text=Hello%20BrightNest%20Academy%2C%20I%20would%20like%20to%20know%20about%20tuition%20classes.";

        private static final Pattern GRADE_PATTERN = Pattern.compile(
                        "(?:grade|class|std|standard)?\\s*(1[0-2]|[1-9])(?:st|nd|rd|th)?", Pattern.CASE_INSENSITIVE);

        private static final List<KnowledgeEntry> KNOWLEDGE_BASE = List.of(
                        new KnowledgeEntry(
                                        "courses",
                                        List.of("course", "courses", "subject", "subjects", "offer", "offering",
                                                        "class", "classes"),
                                        ...
                                        List.of("Do you teach ICSE?", "What grades do you teach?",
                        ...
                        new KnowledgeEntry(
                                        ...
                                        List.of("How do I book a demo class?", "Do you teach online?",
                        ...
                        new KnowledgeEntry(
                                        ...
                                        List.of("What courses do you offer?", "Do you provide online tuition?",
                        ...
                        new KnowledgeEntry(
                                        ...
                                        List.of("What are the fees?", "Do you teach ICSE?",
                                                        "Talk to a human on WhatsApp")),
                        new KnowledgeEntry(
                                        "mode",
                                        ...
                                        List.of("Where is BrightNest Academy located?", "What grades do you teach?",
                                                        "Book a demo class")),
                        new KnowledgeEntry(
                                        "grades",
                                        ...
                                        List.of("Do you teach ICSE?", "What courses do you offer?",
                                                        "Book a demo class")),
                        new KnowledgeEntry(
                                        "maths",
                        ...
                        new KnowledgeEntry(
                                        "location",
                                        List.of("location", "address", "bangalore", "bengaluru", "where", "timing",
                                                        "hours"),
                                        ...
                                        List.of("Get directions", "Do you provide online tuition?",
                                                        "Talk to a human on WhatsApp")),
                        new KnowledgeEntry(
                                        "human-help",
                                        ...
                                        List.of("Talk to a human on WhatsApp", "Book a demo class",
                                                        "What are the fees?")));
        ...
        public Map<String, Object> respond(ChatbotRequest request) {
                ...
                pruneAwaitingGradeSessions();
                String userMessage = InputSanitizer.sanitizeAndTruncate(request.getMessage(), 500);
                String sessionId = resolveSessionId(request.getSessionId());
                ChatbotReply reply = resolveReply(tenantId, sessionId, userMessage);
        ...
        private ChatbotReply resolveReply(Long tenantId, String sessionId, String userMessage) {
                String normalized = userMessage == null ? "" : userMessage.toLowerCase(Locale.ROOT);
                String stateKey = buildSessionStateKey(tenantId, sessionId);
                boolean feeIntent = hasFeeIntent(normalized);
                Integer grade = extractGrade(normalized);

                if (feeIntent && grade == null) {
                        awaitingGradeSessions.put(stateKey, LocalDateTime.now());
                ...
                if (isAwaitingGrade(stateKey) && grade == null) {
                        return new ChatbotReply(
                ...
                if (grade != null && (feeIntent || isAwaitingGrade(stateKey))) {
                        awaitingGradeSessions.remove(stateKey);
                        ...
                        return new ChatbotReply(
                                        ...
                                        "high",
                                        List.of("Book a demo class", "Talk to a human on WhatsApp",
                                                        "Share fee for online classes"),
                                        true);
                }
                ...
                if (bestMatch == null || bestScore < 2) {
                        return new ChatbotReply(
                                        ...
                                        "low",
                                        List.of("What courses do you offer?", "What are the fees?",
                                                        "Talk to a human on WhatsApp"),
                                        false);
                }
        ...
        private String buildGradeRecommendation(int grade, String subject, String board) {
                ...
                if (grade <= 4) {
                        return "For Grade " + grade + ", we recommend our Foundation Program in " + subjectText
                                        + " with concept-building and parent progress updates. This plan is available for "
                                        + boardText + ".";
                }
                if (grade <= 8) {
                        return "For Grade " + grade + ", our Progress Track in " + subjectText
                                        + " focuses on school-aligned learning, weekly tests, and confidence building for "
                                        + boardText + ".";
                }
                return "For Grade " + grade + ", we recommend our Exam Accelerator in " + subjectText
                                + " with chapter-wise tests, revision strategy, and board-focused preparation for "
                                + boardText + ".";
        }
        ...
        private boolean isAwaitingGrade(String sessionStateKey) {
                LocalDateTime lastPromptAt = awaitingGradeSessions.get(sessionStateKey);
                if (lastPromptAt == null) {
                        return false;
                }
                if (Duration.between(lastPromptAt, LocalDateTime.now()).toMinutes() > AWAITING_GRADE_TTL_MINUTES) {
                        awaitingGradeSessions.remove(sessionStateKey);
                        return false;
                }
                return true;
        }

        private String buildSessionStateKey(Long tenantId, String sessionId) {
                return tenantId + ":" + sessionId;
        }

        private void pruneAwaitingGradeSessions() {
                LocalDateTime now = LocalDateTime.now();
                awaitingGradeSessions.entrySet().removeIf(
                                e -> Duration.between(e.getValue(), now).toMinutes() > AWAITING_GRADE_TTL_MINUTES);
                if (awaitingGradeSessions.size() <= MAX_AWAITING_GRADE_SESSIONS) {
                        return;
                }
                int toRemove = awaitingGradeSessions.size() - MAX_AWAITING_GRADE_SESSIONS;
                awaitingGradeSessions.entrySet().stream()
                                .sorted(Map.Entry.comparingByValue())
                                .limit(toRemove)
                                .map(Map.Entry::getKey)
                                .toList()
                                .forEach(awaitingGradeSessions::remove);
        }
