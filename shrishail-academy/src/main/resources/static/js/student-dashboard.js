// Require auth
        ...
        document.getElementById('bigAvatar').textContent = (user.name || 'S')[0].toUpperCase();
        ...
        let allCourses = [];
        let myEnrollments = [];
        const titles = { overview: 'ðŸ“Š Overview', myCourses: 'ðŸ“š My Courses', allCourses: 'ðŸ” Browse Courses', myAttendance: 'âœ… My Attendance', myPayments: 'ðŸ’° My Payments', profile: 'ðŸ‘¤ My Profile' };
        ...
        function renderAllCourses() {
            ...
            if (!allCourses.length) { grid.innerHTML = '<div class="empty-state"><div class="icon">ðŸ“­</div><p>No courses available</p></div>'; return; }
            const enrolledIds = myEnrollments.map(e => e.course ? e.course.id : (e.courseId || null));
            grid.innerHTML = allCourses.map(c => courseCard(c, enrolledIds.includes(c.id))).join('');
        }

        function renderMyCourses() {
            const grid = document.getElementById('myCoursesGrid');
            if (!myEnrollments.length) {
                grid.innerHTML = '<div class="empty-state"><div class="icon">ðŸ“š</div><p>You have not enrolled in any courses yet.</p><br><button onclick="showSection(\'allCourses\')" class="btn-enroll primary" style="width:auto;padding:0.5rem 1.5rem;">Browse Courses</button></div>';
                return;
            }
            grid.innerHTML = myEnrollments.map(e => {
                const c = e.course || {};
        ...
        async function enrollInCourse(courseId, btn) {
            ...
            try {
                ...
                if (!res.ok) { const err = await res.json().catch(() => ({})); throw new Error(err.message || 'Enrollment failed'); }
        ...
        async function loadStudentAttendance() {
            ...
            try {
                ...
                if (document.getElementById('attFilterCourse').options.length <= 1 && myEnrollments.length) {
                ...
                const url = courseId ? `/api/attendance/my-attendance/${courseId}` : '/api/attendance/my-attendance';
                ...
                console.error(e);
                document.querySelector('#myAttendanceTable tbody').innerHTML = '<tr><td colspan="4">Error loading attendance</td></tr>';
            }
        }
        ...
        async function loadStudentPayments() {
            ...
            try {
                const payments = await fetch('/api/payments/my-payments', { headers: hdr }).then(r => r.json());
                ...
                tbody.innerHTML = list.map(p => `
                    ...
                    </tr>`).join('') || '<tr><td colspan="6">No payment records yet</td></tr>';
            } catch (e) {
                console.error(e);
        ...
        async function loadData() {
            try {
                ...
                const [courses, enrollments] = await Promise.all([
                    ...
                    fetch('/api/enrollments/my-courses', { headers: hdr }).then(r => r.json())
                ]);
                ...
                document.getElementById('activeCount').textContent = myEnrollments.filter(e => e.status === 'ACTIVE').length;
                document.getElementById('availableCount').textContent = allCourses.length;

                // Overview courses
                const enrolledIds = myEnrollments.map(e => e.course ? e.course.id : null);
                const oc = document.getElementById('overviewCourses');
                if (!myEnrollments.length) {
                    oc.innerHTML = '<div class="empty-state"><div class="icon">ðŸ“š</div><p>No enrollments yet. <a href="#" onclick="showSection(\'allCourses\')">Browse courses</a></p></div>';
                } else {
                    oc.innerHTML = myEnrollments.slice(0, 3).map(e => courseCard(e.course || {}, true)).join('');
                }
            } catch (e) {
