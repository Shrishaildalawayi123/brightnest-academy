Auth.requireAdmin();
        ...
        let deleteId = null;
        const titles = { overview: 'ðŸ“Š Overview', courses: 'ðŸ“š Manage Courses', students: 'ðŸ‘¥ Students', enrollments: 'ðŸ“‹ Enrollments', attendance: 'âœ… Attendance', payments: 'ðŸ’° Payments', blog: 'ðŸ“ Blog Posts', demos: 'ðŸŽ¯ Demo Bookings', teachers: 'ðŸ§‘â€ðŸ« Teacher Applications' };
        ...
        function showSection(name) {
            ...
            const activeLink = document.querySelector(`.sidebar-link[onclick="showSection('${name}')"]`);
        ...
        function fmt(dt) { return dt ? new Date(dt).toLocaleDateString('en-IN') : '-'; }
        ...
        async function loadOverview() {
            try {
                ...
                document.getElementById('totalEnrollments').textContent = Array.isArray(enrollments) ? enrollments.length : 0;
                document.getElementById('activeEnrollments').textContent = Array.isArray(enrollments) ? enrollments.filter(e => e.status === 'ACTIVE').length : 0;
                const tbody = document.querySelector('#recentStudents tbody');
                tbody.innerHTML = students.slice(0, 5).map(u => `
                    <tr><td><strong>${esc(u.name)}</strong></td><td>${esc(u.email)}</td>
                    <td><span class="badge badge-${u.role.toLowerCase()}">${u.role}</span></td>
                    <td>${fmt(u.createdAt)}</td></tr>`).join('') || '<tr><td colspan="4">No students yet</td></tr>';
        ...
        async function loadCourses() {
            ...
            try {
                ...
                tbody.innerHTML = (Array.isArray(courses) ? courses : []).map(c => `
                    <tr>
                    ...
                    </tr>`).join('') || '<tr><td colspan="5" class="empty-state">No courses found</td></tr>';
            } catch (e) { tbody.innerHTML = '<tr><td colspan="4">Error loading courses</td></tr>'; }
        }
        ...
        async function loadStudents() {
            ...
            try {
                const users = await fetch('/api/users', { headers: authHdr }).then(r => r.json());
            ...
            } catch (e) { tbody.innerHTML = '<tr><td colspan="4">Error loading students</td></tr>'; }
        }
        ...
        async function loadEnrollments() {
            ...
            try {
                const enrollments = await fetch('/api/enrollments', { headers: authHdr }).then(r => r.json());
                tbody.innerHTML = (Array.isArray(enrollments) ? enrollments : []).map(e => `
                    <tr>
                    ...
                    </tr>`).join('') || '<tr><td colspan="4">No enrollments yet</td></tr>';
            } catch (e) { tbody.innerHTML = '<tr><td colspan="4">Error loading enrollments</td></tr>'; }
        }
        ...
        document.getElementById('courseForm').addEventListener('submit', async (e) => {
            ...
            const data = {
                ...
                fee: document.getElementById('courseFee').value ? parseFloat(document.getElementById('courseFee').value) : null
            };
        ...
        function deleteCourse(id, name) {
            deleteId = id;
            document.getElementById('deleteCourseName').textContent = `"${name}" will be permanently deleted.`;
            document.getElementById('deleteModal').classList.add('show');
        }

        async function confirmDelete() {
            try {
                await fetch(`/api/courses/${deleteId}`, { method: 'DELETE', headers: authHdr });
        ...
        async function loadAttendanceSection() {
            ...
            try {
                ...
                sel.innerHTML = '<option value="">Select Course</option>' +
                    (Array.isArray(courses) ? courses : []).map(c => `<option value="${c.id}">${esc(c.title)}</option>`).join('');
            } catch (e) { console.error(e); }
        ...
        async function loadCourseStudents() {
            ...
            if (!courseId) { document.getElementById('attendanceForm').style.display = 'none'; return; }
            try {
                const enrollments = await fetch('/api/enrollments', { headers: authHdr }).then(r => r.json());
                attStudents = (Array.isArray(enrollments) ? enrollments : [])
                    .filter(e => e.course && e.course.id == courseId && e.status === 'ACTIVE');
                const tbody = document.querySelector('#attendanceTable tbody');
                if (attStudents.length === 0) {
                    tbody.innerHTML = '<tr><td colspan="3">No active students enrolled in this course</td></tr>';
                } else {
                    tbody.innerHTML = attStudents.map(e => `
                        <tr>
        ...
        async function submitAttendance() {
            ...
            if (!courseId || !date) { showToast('Select course and date', 'error'); return; }

            const records = [];
            document.querySelectorAll('.att-status').forEach(sel => {
                const sid = sel.dataset.studentId;
                const remarks = document.querySelector(`.att-remarks[data-student-id="${sid}"]`).value;
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    loadRecentAttendance();
                } else { showToast(data.message || 'Failed to mark attendance', 'error'); }
            } catch (e) { showToast('Error submitting attendance', 'error'); }
        }

        async function loadRecentAttendance() {
            const tbody = document.querySelector('#recentAttendance tbody');
            try {
                const records = await fetch('/api/attendance', { headers: authHdr }).then(r => r.json());
                ...
                tbody.innerHTML = list.map(a => `
                    <tr>
                        ...
                        <td>${esc(a.remarks || '-')}</td>
                    </tr>`).join('') || '<tr><td colspan="5">No attendance records yet</td></tr>';
            } catch (e) { tbody.innerHTML = '<tr><td colspan="5">Error loading records</td></tr>'; }
        }
        ...
        async function loadPaymentStats() {
            try {
                ...
                document.getElementById('successPayments').textContent = stats.successCount || 0;
                document.getElementById('pendingPayments').textContent = stats.pendingCount || 0;
                document.getElementById('failedPayments').textContent = stats.failedCount || 0;
            } catch (e) { console.error(e); }
        }
        ...
        async function loadPaymentDropdowns() {
            try {
                ...
                const students = (Array.isArray(users) ? users : []).filter(u => u.role === 'STUDENT');
                document.getElementById('payStudent').innerHTML = '<option value="">Select Student</option>' +
                    students.map(s => `<option value="${s.id}">${esc(s.name)} (${esc(s.email)})</option>`).join('');
                document.getElementById('payCourse').innerHTML = '<option value="">Select Course</option>' +
                    (Array.isArray(courses) ? courses : []).map(c => `<option value="${c.id}" data-fee="${c.fee || 0}">${esc(c.title)} - â‚¹${c.fee || 0}</option>`).join('');
            } catch (e) { console.error(e); }
        }
        ...
        document.getElementById('payCourse').addEventListener('change', function() {
            ...
            if (opt.dataset.fee) document.getElementById('payAmount').value = opt.dataset.fee;
        });
        ...
        async function recordManualPayment() {
            ...
            if (!userId || !courseId || !amount) { showToast('Fill all required fields', 'error'); return; }

            try {
                const res = await fetch(`/api/payments/manual/${userId}`, {
                    method: 'POST', headers: authHdr,
                    body: JSON.stringify({ courseId: parseInt(courseId), amount: parseFloat(amount), paymentMethod: method })
                });
                const data = await res.json();
                if (res.ok) {
                    ...
                    loadPaymentStats();
                    loadAllPayments();
                } else { showToast(data.message || 'Failed to record payment', 'error'); }
        ...
        async function loadAllPayments() {
            ...
            try {
                const payments = await fetch('/api/payments', { headers: authHdr }).then(r => r.json());
                tbody.innerHTML = (Array.isArray(payments) ? payments : []).map(p => `
                    <tr>
                    ...
                    </tr>`).join('') || '<tr><td colspan="8">No payments yet</td></tr>';
            } catch (e) { tbody.innerHTML = '<tr><td colspan="8">Error loading payments</td></tr>'; }
        }

        async function confirmPayment(paymentId) {
            try {
                const res = await fetch(`/api/payments/${paymentId}/confirm`, { method: 'POST', headers: authHdr });
                if (res.ok) { showToast('Payment confirmed!'); loadPaymentStats(); loadAllPayments(); }
                else { showToast('Failed to confirm payment', 'error'); }
            } catch (e) { showToast('Error', 'error'); }
        }

        async function failPaymentAction(paymentId) {
            try {
                const res = await fetch(`/api/payments/${paymentId}/fail?reason=Admin%20rejected`, { method: 'POST', headers: authHdr });
                if (res.ok) { showToast('Payment marked as failed'); loadPaymentStats(); loadAllPayments(); }
        ...
        async function loadBlogAdmin() {
            ...
            try {
                const posts = await fetch('/api/blog/all', { headers: authHdr }).then(r => r.json());
                tbody.innerHTML = (Array.isArray(posts) ? posts : []).map(p => `
                    <tr>
                    ...
                    </tr>`).join('') || '<tr><td colspan="5">No blog posts yet</td></tr>';
            } catch(e) { tbody.innerHTML = '<tr><td colspan="5">Error loading posts</td></tr>'; }
        }

        async function togglePublish(id) {
            try {
                await fetch(`/api/blog/${id}/publish`, { method: 'PUT', headers: authHdr });
                showToast('Publish status toggled'); loadBlogAdmin();
            } catch(e) { showToast('Error toggling publish', 'error'); }
        }
        ...
        async function deleteBlogPost(id) {
            ...
            try {
                ...
                showToast('Blog post deleted'); loadBlogAdmin();
        ...
        async function saveBlogPost(e) {
            ...
            try {
                const res = await fetch('/api/blog', { method: 'POST', headers: authHdr, body: JSON.stringify(data) });
                if (res.ok) { showToast('Blog post created!'); document.getElementById('blogModal').remove(); loadBlogAdmin(); }
                else { const err = await res.json(); showToast(err.message || 'Error creating post', 'error'); }
        ...
        async function loadDemosAdmin() {
            try {
                const [bookings, stats] = await Promise.all([
                    ...
                    fetch('/api/demo-booking/stats', { headers: authHdr }).then(r => r.json())
                ]);
                document.getElementById('totalDemos').textContent = (stats.PENDING||0)+(stats.SCHEDULED||0)+(stats.COMPLETED||0)+(stats.CANCELLED||0);
                document.getElementById('pendingDemos').textContent = stats.PENDING||0;
                document.getElementById('scheduledDemos').textContent = stats.SCHEDULED||0;
                document.getElementById('completedDemos').textContent = stats.COMPLETED||0;
                const tbody = document.querySelector('#demosTable tbody');
                tbody.innerHTML = (Array.isArray(bookings) ? bookings : []).map(d => `
                    <tr>
                    ...
                    </tr>`).join('') || '<tr><td colspan="8">No demo bookings yet</td></tr>';
            } catch(e) { console.error(e); document.querySelector('#demosTable tbody').innerHTML = '<tr><td colspan="8">Error loading</td></tr>'; }
        }

        async function updateDemoStatus(id, status) {
            try {
                await fetch(`/api/demo-booking/${id}/status?status=${status}`, { method: 'PUT', headers: authHdr });
                showToast('Status updated'); loadDemosAdmin();
        ...
        async function loadTeacherAppsAdmin() {
            try {
                const [apps, stats] = await Promise.all([
                    fetch('/api/teacher-applications', { headers: authHdr }).then(r => r.json()),
                    fetch('/api/teacher-applications/stats', { headers: authHdr }).then(r => r.json())
                ]);
                document.getElementById('totalTeacherApps').textContent = (stats.NEW||0)+(stats.REVIEWED||0)+(stats.CONTACTED||0)+(stats.HIRED||0)+(stats.REJECTED||0);
                ...
                tbody.innerHTML = (Array.isArray(apps) ? apps : []).map(a => `
                    <tr>
            ...
            } catch(e) { console.error(e); document.querySelector('#teacherAppsTable tbody').innerHTML = '<tr><td colspan="8">Error loading</td></tr>'; }
        }

        async function updateTeacherStatus(id, status) {
            try {
                await fetch(`/api/teacher-applications/${id}/status?status=${status}`, { method: 'PUT', headers: authHdr });
                showToast('Status updated'); loadTeacherAppsAdmin();
            } catch(e) { showToast('Error updating status', 'error'); }
        }
